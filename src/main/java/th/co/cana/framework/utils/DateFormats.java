...
public final class DateFormats {
	...
	private static final CustomDateFormat[] FM_DATES;
	static {
		FM_DATES = new CustomDateFormat[6];
		FM_DATES[0] = createFormat(FM_DATE);
		FM_DATES[1] = createFormat("yyyy-MM-dd");
		FM_DATES[2] = createFormat(FM_DT_TIME);
		FM_DATES[3] = createFormat("yyyy-MM-dd HH:mm:ss");
		FM_DATES[4] = createFormat("yyyy-MM-dd HH:mm:ss.SSS");
		FM_DATES[5] = createFormat("dd/MM/yyyy HH:mm:ss.SSS");
	}
	...
	private static Date toDate(String value) {
		...
		for (CustomDateFormat fmt : FM_DATES) {
			if (fmt.lengthPattern() == value.trim().length()) {
				Date date = parse(fmt, value);
				if (date != null) {
					return date;
				}
			}
		...
		return parse(FM_DATES[0], value);
	}
	
	private static CustomDateFormat createFormat(String pattern) { 
		CustomDateFormat dfm = new CustomDateFormat(pattern);
		dfm.setLenient(false);		
		return dfm;
	}
	
	private static Date parse(CustomDateFormat fmt, String value) {
		try {
